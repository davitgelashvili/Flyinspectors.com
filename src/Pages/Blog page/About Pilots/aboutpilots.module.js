module.exports = {
    en: {
        aboutpilots: {
            header: `10 Interesting Facts About Pilots`,
            text: `There are probably no people who would deny the importance of aviation
          in today’s fast moving world. The question is how much do we really
          know about aviation and pilots ? Undoubtedly, there are countless
          interesting and surprising facts about pilots which may surprise you.
          That is the reason why we have chosen to come up with the list of the
          most astonishing facts about pilots. Who knows, maybe after reading
          this article you will be able to add some new facts about aviation and
          pilot profession to your knowledge pool.`,
        }

    },
    ka: {
        aboutpilots: {
            header: `10 საინტერესო ფაქტი პილოტების შესახებ`,
            text: `ალბათ არ არსებობს ხალხი, ვინც უარყოფს ავიაციის მნიშვნელობას
           დღევანდელ სწრაფად მოძრავ სამყაროში. საკითხავია, რეალურად რამდენს ვაკეთებთ
           იცით ავიაციის და პილოტების შესახებ? უდავოა, უთვალავია
          საინტერესო და გასაკვირი ფაქტები მფრინავების შესახებ, რომლებიც შეიძლება გაგიკვირდეთ.
         ეს არის მიზეზი, რის გამოც ჩვენ ავირჩიეთ შეგვექმნა სია
         ყველაზე გასაოცარი ფაქტები მფრინავების შესახებ. ვინ იცის, იქნებ წაკითხვის შემდეგ
         ამ სტატიაში თქვენ შეძლებთ დაამატოთ რამდენიმე ახალი ფაქტი ავიაციის შესახებ და
          პილოტის პროფესია თქვენი ცოდნის ფონდში.`,
        }

    }
}